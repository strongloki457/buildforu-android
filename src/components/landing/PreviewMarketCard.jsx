  ...
  return (
    <article className="rounded-[26px] border border-slate-100 bg-white p-5 shadow-soft">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm text-slate-500">{t("landing.preview.market.label")}</p>
          <p className="text-base text-slate-900">{t("landing.preview.market.title")}</p>
        </div>
      </div>
      ...
      <div className="mt-3 space-y-3">
        {previewMarketResults.map((resultKey) => (
          <div key={resultKey} className="rounded-2xl bg-slate-50 px-3 py-3">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm text-slate-900">{t(`landing.preview.market.${resultKey}.store`)}</p>
